...
namespace UltimaOnline.Ethics.Evil
{
    public sealed class UnholyItem : Power
    {
        ...
        private void Power_OnTarget(Mobile fromMobile, object obj, object state)
        {
            ...
            if (item == null)
            {
                from.Mobile.LocalOverheadMessage(UltimaOnline.Network.MessageType.Regular, 0x3B2, false, "You may not imbue that.");
                return;
            }

            if (item.Parent != from.Mobile)
            {
                from.Mobile.LocalOverheadMessage(UltimaOnline.Network.MessageType.Regular, 0x3B2, false, "You may only imbue items you are wearing.");
                return;
            }

            if ((item.SavedFlags & 0x300) != 0)
            {
                from.Mobile.LocalOverheadMessage(UltimaOnline.Network.MessageType.Regular, 0x3B2, false, "That has already beem imbued.");
                return;
            }
