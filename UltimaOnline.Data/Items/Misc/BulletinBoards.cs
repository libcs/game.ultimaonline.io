...
namespace UltimaOnline.Items
{
    ...
    public abstract class BaseBulletinBoard : Item
    {
        ...
        static TimeSpan ThreadDeletionTime = TimeSpan.FromHours(6.0);

        // A player may only create a thread once every two minutes
        static TimeSpan ThreadCreateTime = TimeSpan.FromMinutes(2.0);

        // A player may only reply once every thirty seconds
        static TimeSpan ThreadReplyTime = TimeSpan.FromSeconds(30.0);
    ...
    public class BulletinMessage : Item
    {
        ...
        public override bool CheckTarget(Mobile from, UltimaOnline.Targeting.Target targ, object targeted)
        {
            return false;
        }
    ...
    public class BBMessageHeader : Packet
    {
        public BBMessageHeader(BaseBulletinBoard board, BulletinMessage msg) : base(0x71)
        {
            ...
            if (thread == null)
                ...
                Stream.Write((int)thread.Serial); // Thread serial--parent
