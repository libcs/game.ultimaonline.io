...
namespace UltimaOnline
{
    public class AssemblyEmitter
    {
        ...
        public TypeBuilder DefineType(string typeName, TypeAttributes attrs, Type parentType) => _ModuleBuilder.DefineType(typeName, attrs, parentType);

        public void Save() => throw new NotImplementedException(); // _AssemblyBuilder.Save($"{_AssemblyName.ToLower()}.dll");
    }
    ...
    public class MethodEmitter
    {
        ...
        Stack<Type> _Stack;
        Stack<CallInfo> _Calls;
        Dictionary<Type, Queue<LocalBuilder>> _Temps;
        ...
        {
            Type = typeBuilder;
            _Temps = new Dictionary<Type, Queue<LocalBuilder>>();
            _Stack = new Stack<Type>();
            _Calls = new Stack<CallInfo>();
        }
        ...
        {
            if (!_Temps.TryGetValue(localType, out var list))
                _Temps[localType] = list = new Queue<LocalBuilder>();
        ...
        public void ReleaseTemp(LocalBuilder local)
        {
            ...
            if (!_Temps.TryGetValue(local.LocalType, out var list))
                _Temps[local.LocalType] = list = new Queue<LocalBuilder>();
            list.Enqueue(local);
        }

        public void Branch(Label label) => Generator.Emit(OpCodes.Br, label);
        public void BranchIfFalse(Label label) { Pop(typeof(object)); Generator.Emit(OpCodes.Brfalse, label); }
        public void BranchIfTrue(Label label) { Pop(typeof(object)); Generator.Emit(OpCodes.Brtrue, label); }
        ...
        public void Push(Type type) => _Stack.Push(type);

        public void Return()
        {
            if (_Stack.Count != (Method.ReturnType == typeof(void) ? 0 : 1))
                throw new InvalidOperationException("Stack return mismatch.");
            Generator.Emit(OpCodes.Ret);
        }

        public void LoadNull() => LoadNull(typeof(object));
        public void LoadNull(Type type) { Push(type); Generator.Emit(OpCodes.Ldnull); }
        ...
        public void Load(long value) { Push(typeof(long)); Generator.Emit(OpCodes.Ldc_I8, value); }
        public void Load(float value) { Push(typeof(float)); Generator.Emit(OpCodes.Ldc_R4, value); }
        public void Load(double value) { Push(typeof(double)); Generator.Emit(OpCodes.Ldc_R8, value); }
        public void Load(char value) { Load((int)value); Pop(); Push(typeof(char)); }

        public void Load(bool value)
        {
            Push(typeof(bool));
            if (value) Generator.Emit(OpCodes.Ldc_I4_1);
        ...
        public void Load(int value)
        {
            ...
            switch (value)
            {
                ...
                default:
                    if (value >= sbyte.MinValue && value <= sbyte.MaxValue) Generator.Emit(OpCodes.Ldc_I4_S, (sbyte)value);
                    else Generator.Emit(OpCodes.Ldc_I4, value);
                    break;
            }
        }
        ...
        public void LoadLocal(LocalBuilder local)
        {
            ...
            switch (index)
            {
                ...
                default:
                    if (index >= byte.MinValue && index <= byte.MinValue) Generator.Emit(OpCodes.Ldloc_S, (byte)index);
                    else Generator.Emit(OpCodes.Ldloc, (short)index);
                    break;
            }
        }

        public void StoreLocal(LocalBuilder local) { Pop(local.LocalType); Generator.Emit(OpCodes.Stloc, local); }

        public void LoadArgument(int index)
        {
            Push(index > 0 ? _ArgumentTypes[index - 1] : Type);
            switch (index)
            {
                ...
                default:
                    if (index >= byte.MinValue && index <= byte.MaxValue) Generator.Emit(OpCodes.Ldarg_S, (byte)index);
                    else Generator.Emit(OpCodes.Ldarg, (short)index);
                    break;
            }
        }

        public void CastAs(Type type) { Pop(typeof(object)); Push(type); Generator.Emit(OpCodes.Isinst, type); }
        public void Neg() { Pop(typeof(int)); Push(typeof(int)); Generator.Emit(OpCodes.Neg); }
        public void Compare(OpCode opCode) { Pop(); Pop(); Push(typeof(int)); Generator.Emit(opCode); }
        public void LogicalNot() { Pop(typeof(int)); Push(typeof(int)); Generator.Emit(OpCodes.Ldc_I4_0); Generator.Emit(OpCodes.Ceq); }
        public void Xor() { Pop(typeof(int)); Pop(typeof(int)); Push(typeof(int)); Generator.Emit(OpCodes.Xor); }

        public Type Active => _Stack.Peek();
        ...
        public bool CompareTo(int sign, Callback argGenerator)
        {
            ...
            if (compareTo == null)
            {
				 ...
				 * Bleh.
				 */
                var ifaces = active.FindInterfaces((Type type, object obj) => type.IsGenericType && type.GetGenericTypeDefinition() == typeof(IComparable<>) && type.GetGenericArguments()[0].IsAssignableFrom(active), null);
                if (ifaces.Length > 0)
                    compareTo = ifaces[0].GetMethod("CompareTo", new Type[] { active });
                else
                {
                    ifaces = active.FindInterfaces((Type type, object obj) => type == typeof(IComparable), null);
                    if (ifaces.Length > 0)
                        compareTo = ifaces[0].GetMethod("CompareTo", new Type[] { active });
                }
            }
        ...
        {
            var type = (method.CallingConvention & CallingConventions.HasThis) != 0 ? _Stack.Peek() : method.DeclaringType;
            _Calls.Push(new CallInfo(type, method));
            if (type.IsValueType)
            {
        ...
        {
            var call = _Calls.Pop();
            if ((call.type.IsValueType || call.type.IsByRef) && call.method.DeclaringType != call.type) Generator.Emit(OpCodes.Constrained, call.type);
            if (call.method.DeclaringType.IsValueType || call.method.IsStatic) Generator.Emit(OpCodes.Call, call.method);
            else Generator.Emit(OpCodes.Callvirt, call.method);
