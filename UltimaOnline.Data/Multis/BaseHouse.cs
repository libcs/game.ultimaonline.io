...
using UltimaOnline.Multis.Deeds;
...
using UltimaOnline.Engines.BulkOrders;
...
namespace UltimaOnline.Multis
{
    public abstract class BaseHouse : BaseMulti
    {
        ...
        public void Release(Mobile m, Item item)
        {
            ...
            if (IsLockedDown(item))
            {
                item.PublicOverheadMessage(UltimaOnline.Network.MessageType.Label, 0x3B2, 501657);//[no longer locked down]
                SetLockdown(item, false);
            ...
            for (int i = 0; i < m_Secures.Count; ++i)
            {
                ...
                if (info.Item == item && HasSecureAccess(m, info.Level))
                {
                    ...
                    if (item is BaseAddonContainer)
                        ...
                        item.Movable = true;
                    item.SetLastMoved();
                    item.PublicOverheadMessage(UltimaOnline.Network.MessageType.Label, 0x3B2, 501656);//[no longer secure]
