			...
			if ( IsOpen )
			{
				...
				for ( int i = 1; i <= 6; ++i )
				{
					...
					for ( int j = -8; j <= 8; ++j )
					{
						...
						if ( map.CanFit( x, y, z, 16, false, false ) && !UltimaOnline.Spells.SpellHelper.CheckMulti( new Point3D( x, y, z ), map ) && !Region.Find( new Point3D( x, y, z ), map ).IsPartOf( typeof( Factions.StrongholdRegion ) ) )
						{
							if ( i == 1 && j >= -2 && j <= 2 )
								return true;
