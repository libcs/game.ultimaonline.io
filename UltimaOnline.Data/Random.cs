...
namespace UltimaOnline
{
    ...
    {
        static readonly IRandomImpl _Random;

        static RandomImpl()
        {
            if (Core.Unix) _Random = new CSPRandom();
            else if (Core.Is64Bit && File.Exists("rdrand64.dll")) _Random = new RDRand64();
            else if (!Core.Is64Bit && File.Exists("rdrand32.dll")) _Random = new RDRand32();
            else _Random = new CSPRandom();
    ...
    public sealed class SimpleRandom : IRandomImpl
    {
        ...
        public bool NextBool() => NextDouble() >= .5;
    ...
    {
        RNGCryptoServiceProvider _CSP = new RNGCryptoServiceProvider();
        static int BUFFER_SIZE = 0x4000;
        static int LARGE_REQUEST = 0x40;
        byte[] _Working = new byte[BUFFER_SIZE];
        byte[] _Buffer = new byte[BUFFER_SIZE];
        int _Index = 0;
        object _sync = new object();
        ManualResetEvent _filled = new ManualResetEvent(false);
        ...
        public int Next(int c) => (int)(c * NextDouble());

        public bool NextBool() => (NextByte() & 1) == 1;
